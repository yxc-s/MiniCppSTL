...
class set_impl: public mstl::RedBlackTree<T, T, COMPARE_FUNCTION, Allocator, false, IS_MULTI> {
    ...
    using this_set_type = set_impl<T, COMPARE_FUNCTION, Allocator, IS_MULTI>;
public:
    ...
    set_impl(this_set_type&& other) : tree_base(mstl::move(other)) {}
    ...
    /* =移动语义*/
    this_set_type& operator =(this_set_type&& other) {
        tree_base::destory(tree_base::root_);
